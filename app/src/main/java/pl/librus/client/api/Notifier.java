...
class Notifier {
    ...
    Notifier(Context context, LibrusData data) {
        this.context = context;
        this.data = data;
    }

    private void sendNotification(@NonNull CharSequence title, @NonNull CharSequence text, int iconResource, @Nullable CharSequence subtext, @Nullable Notification.Style style) {
    ...
    void addAnnouncements(List<Announcement> announcements) {
        ...
        if (size == 1) {
            ...
            sendNotification(title, TextUtils.join(", ", authors), R.drawable.ic_announcement_black_48dp, null, style);
        }
    }
    ...
    void addGrades(List<Grade> grades) {
        ...
        if (size == 1) {
            ...
            sendNotification("Nowa ocena", subject + " " + grade.getGrade(), R.drawable.ic_assignment_black_48dp, null, null);
        } else if (size > 1) {
            String title;
...
//                String category = data.getGradeCategoriesMap().get(g.getCategoryId()).getName();
    ...
    void addEvents(List<Event> events) {
        ...
        if (size == 1) {
            ...
            for (Event e : events) {
                ...
                if (!subjects.contains(getEventSubject(e).getName()))
                    subjects.add(getEventSubject(e).getName());
            }
            sendNotification(title,
                    TextUtils.join(", ", subjects),
                    R.drawable.ic_event_black_24dp,
    ...
    void addLuckyNumber(LuckyNumber ln) {
        sendNotification(
                "Wylosowano szczęśliwy numerek: " + ln.getLuckyNumber(),
                ln.getLuckyNumberDay().toString("EEEE, d MMMM yyyy", new Locale("pl")),
                R.drawable.ic_sentiment_very_satisfied_black_24dp,
                null, null);
    }

    private Subject getEventSubject(Event e) {
        return data.getTimetable().getLesson(e.getDate().withWeekOfWeekyear(LocalDate.now().getWeekOfWeekyear()), e.getLessonNumber()).getSubject();
    }
}
